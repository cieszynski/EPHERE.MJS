...
const hhmm = (hhdec) => { return [String(trunc(hhdec)).padStart(2, 0), String(round(hhdec % 1 / (1 / 60))).padStart(2, 0)]; }
...
function test() {
    ...
    function test_sun() {
        ...
        let xx = sun.times(jd, 1, lat, lon)
        log(hhmm(xx.rise).join(':'), "(4:44)",
            hhmm(xx.set).join(':'), "(21:14)")

        xx = sun.times(jd + 10, 1, lat, lon) // 2020, 6, 18
        log(hhmm(xx.rise).join(':'), "(4:42)",
            hhmm(xx.set).join(':'), "(21:20)");
        ...
        log(hhmm(UT * 24).join(':'), "(11:59)",
            (az * deg).toFixed(2), "(180°)",
            (al * deg).toFixed(2), "(61°)")

        UT = 3.745 / 24; // 3:44 
        LHA = sun.local_hour_angle(jd, UT, lon)
        az = sun.azimuth(LHA, lat, pos.dec);
        al = sun.altitude(LHA, lat, pos.dec)
        log(hhmm(UT * 24).join(':'), "(3:44)",
            (az * deg).toFixed(2), "(50°)",
            (al * deg).toFixed(2), "(0°)")
    }
    ...
    function test_moon() {
        ...
        let xx = moon.times(jd, 1, lat, lon, 8 / 60)
        log(hhmm(xx.rise).join(':'), "(-:--)",
            hhmm(xx.set).join(':'), "(7:01)")

        xx = moon.times(jd + 10, 1, lat, lon, 8 / 60) // 2020, 6, 18
        log(hhmm(xx.rise).join(':'), "(3:12)",
            hhmm(xx.set).join(':'), "(18:26)")

        UT = 6.025 / 24; // 6:01
        let pos = moon.equatorial_coordinates(jd);
        log("R", pos.r)
